...
def data_download(type_to_download, data_list_to_download, url_year="", year=0, dem_n="", year_list=None,
    ...
    for i in data_list_to_download:
        ...
        file_name = ""
        if type_to_download == "meta_data" or "auxiliary_data":
            if i == "url_id_data":
                url = "https://raw.githubusercontent.com/Jon-Fr/Geo_419b/main/url_id_file.zip?token=" \
                      "ANQNA6GYPM3LPVQLVUFK6Y3BOPI5A"
                file_name = "url_id_file.csv"
            else:
                url = """https://geoportal.geoportal-th.de/hoehendaten/Uebersichten/Stand_{}.zip""".format(url_year)
            data_year = ""
            if type_to_download == "aux_data":
        ...
        if type_to_download == "ortho":
            ...
            file_name = tile_number_list[index] + "_1_th_" + str(year_list[index])
            index = index + 1
    ...
    if ortho is True:
        ...
        if image_data != "no_new_data":
            ...
            for zip_file_name in image_data:
                create_and_unzip(folder_path="image_data/orthophotos/"+str(year_list[index]), zip_files=[zip_file_name])
                index = index + 1
